...
mdat <- dat %>%
  ...
  mutate(deg.prg = as.factor(deg.prg),
...
Younger alumni have given less, which follows given their lower lifetime earnings. Part-time alumni appear to be less philanthropic than the others.
...
ggplot(rf.imp, aes(x=rownames(rf.imp), y=X.IncMSE)) + geom_bar(stat="identity", alpha=.5) + geom_text(aes(x=rownames(rf.imp), y=X.IncMSE+.01, ymax=X.IncMSE, label=round(X.IncMSE, 2)), size=3, alpha=.5) + theme(axis.text.x=element_text(angle=90, hjust=1)) + labs(title="Variable importances", x="Variable", y="Increase in MSE")
```
...
Now the engagement indicators really pop. For modeling, `stewardee` and `gift.cap` should certainly be dropped, and `first.gift` is borderline, as it's perfectly correlated with the response variable `student.gifts` in many cases as seen during exploration above.
