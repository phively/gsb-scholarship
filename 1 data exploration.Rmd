...
```{r, message=F, warning=F, cache=F}
# Remove unused variables
lmdat <- mdat %>% filter(student.gifts > 0) %>% select(-student.gifts, -stewardee, -gift.cap, -first.gift)
# Recenter deg.year
...
lmod <- lm(
  student.gifts.lt ~
    ...
    # Sqrt transform
    sqrt(employ.yrs) + sqrt(fys.giving) + sqrt(af.giving) + sqrt(known.to) + sqrt(committees) +
...
```{r, echo=F, message=F, warning=F, cache=F}
# Prediction data
predsdat <- mdat %>% mutate(emba = deg.prg=="EMBA", deg.year.ctr = deg.year - min(lmdat$deg.year))
# Predict on full dataset
preds <- predict(lmod2, newdata=predsdat)

# Plot expected versus actual
ggdat <- data.frame(Predicted = 10^preds, Actual = predsdat$student.gifts)
ggdat %>% ggplot(aes(x=Predicted, y=Actual)) + geom_point(alpha=.1) + gg.y.logdollar + gg.x.logdollar + labs(title="Predicted versus actual giving")
```

When including non-donors, the predicted amounts are biased high, unsurprisingly. The final step is to join the predictions and unique identifier and export the data. For this first pass, it's helpful to have the data in Excel for others to explore.

```{r, message=F, warning=F, cache=F}
# Save the id and prediction columns
out <- predsdat %>% select(id) %>% mutate(preds=preds)
